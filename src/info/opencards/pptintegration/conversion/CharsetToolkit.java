...
/**
 * <p>Utility class to guess the encoding of a given byte array. The guess is unfortunately not 100% sure. Especially
 * for 8-bit charsets. It's not possible to know which 8-bit charset is used. Except through statistical analysis. We
 * will then infer that the charset encountered is the same as the default standard charset.</p> <p/> <p>On the other
 * hand, unicode files encoded in UTF-16 (low or big endian) or UTF-8 files with a Byte Order Marker are easy to find.
 * For UTF-8 files with no BOM, if the buffer is wide enough, it's easy to guess.</p> <p/> <p>Tested against a
 * complicated UTF-8 file, Sun's implementation does not render bad UTF-8 constructs as expected by the specification.
 * But with a buffer wide enough, the method guessEncoding() did behave correctly and recognized the UTF-8 charset.</p>
 * <p/> <p>A byte buffer of 4KB or 8KB is sufficient to be able to guess the encoding.</p> <p/> <p>Usage:</p>
 * <pre>
 * // guess the encoding
...
public class CharsetToolkit {
     ...
     */
    private CharsetToolkit(byte[] buffer) {
        this.buffer = buffer;
    ...
    /**
     * <p>Guess the encoding of the provided buffer.</p> If Byte Order Markers are encountered at the beginning of the
     * buffer, we immidiately return the charset implied by this BOM. Otherwise, the file would not be a human readable
     * text file.</p> <p/> <p>If there is no BOM, this method tries to discern whether the file is UTF-8 or not. If it
     * is not UTF-8, we assume the encoding is the default system encoding (of course, it might be any 8-bit charset,
     * but usually, an 8-bit charset is the default one).</p> <p/> <p>It is possible to discern UTF-8 thanks to the
     * pattern of characters with a multi-byte sequence.</p>
     * <pre>
    ...
    public Charset guessEncoding() {
        ...
        // note the buffer is not read up to the end, but up to length - 6
    ...
    private static Charset guessEncoding(File f, int bufferLength) throws IOException {
     ...
     */
    private static Charset getDefaultSystemCharset() {
